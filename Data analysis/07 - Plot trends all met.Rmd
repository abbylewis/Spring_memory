...
- Step 4: Repeat for bottom-water DO
- Step 5: Repeat for surface temperature
- Step 6: Repeat for VHOD
- Step 7: Generate plot of bottom water temperature trend drivers (spring vs summer temp)
...
summer_dif_do <- read.csv("../Compiled data/summer_dif_DO_trends_3m.csv") %>%
  rename(summer_dif = trend) %>%
  dplyr::select(summer_dif, LakeID)
summer_vhod <- read.csv("../Compiled data/summer_vhod_trends_3m.csv") %>%
  rename(summer_vhod = trend) %>%
  dplyr::select(summer_vhod, LakeID)
```
...
c <- bot %>%
  ...
  left_join(dif_rf) %>%
  filter(!is.na(M)) %>%
  mutate(M_binary = ifelse(M>0, "CSEM > 0", "CSEM ≤ 0"),
...
jpeg("../Figures/Figure 6.jpg", width = 6, height = 4, units = "in", res = 600)
...
jpeg("../Figures/Figure 5.jpg", res = 600, units = "in", width = 5, height = 3)
...
cor_plot <- mem_rf_df_bot_do %>%
  pivot_longer(c(Air_temp_C_spring, Air_temp_C_summer)) %>%
  mutate(name = case_match(name,
                          "Air_temp_C_spring"~"Spring",
                          "Air_temp_C_summer"~"Summer")) %>%
  group_by(name) %>%
  mutate(rho = cor.test(value, summer_bot_do*10, method = "spearman")$estimate,
         p = cor.test(value, summer_bot_do*10, method = "spearman")$p.value,
         p_lab = ifelse(p > 0.001, round(p, 2), p),
         p_lab = paste0("= ", round(p_lab, 3)),
         p_lab = ifelse(p_lab == "= 0",
                        "< 0.001",
                        p_lab)) %>%
  ggplot(aes(x = value*10, y = summer_bot_do*10))+
  geom_vline(aes(xintercept = 0), color = "grey70")+
  geom_point(shape = 21, stroke = 0.3)+
  #geom_abline()+
  #geom_smooth(method = "lm")+
  geom_label(aes(label = paste0("Spearman's \u03C1 = ", round(rho, 2),
                                "\np ", p_lab), x = -1.7, y = 3.5, hjust= 0), 
             data = . %>% select(name, rho, p_lab) %>% distinct(), alpha = 0.5)+
  facet_grid(~name)+
  theme_bw()+
  xlab("Air temperature trend (ºC/decade)")+
  ylab("Bottom-water dissolved\noxygen trend (ºC/decade)")

air_trends_wide_data <- mem_rf_df_bot_do %>%
  pivot_longer(cols = all_of(potential_drivers),
               names_to = "var", values_to = "x") %>%
  filter(!is.na(x)) %>%
  mutate(var = factor(var, levels = c("Air_temp_C_spring",
...
jpeg("../Figures/simple_Fig5_DO.jpg", res = 300, units = "in", width = 5, height = 3)
ggpubr::ggarrange(air_trends_wide_blank + theme(plot.margin = unit(c(.2,.2,0,1), 'lines')), 
                    cor_plot + theme(plot.margin = unit(c(0,.2,.2,1), 'lines')), 
                    heights = c(0.2, 1), nrow = 2)
dev.off()
...
cor_plot <- mem_rf_df_surf_temp %>%
  pivot_longer(c(Air_temp_C_spring, Air_temp_C_summer)) %>%
  mutate(name = case_match(name,
                          "Air_temp_C_spring"~"Spring",
                          "Air_temp_C_summer"~"Summer")) %>%
  group_by(name) %>%
  mutate(rho = cor.test(value, summer_surf_mean*10, method = "spearman")$estimate,
         p = cor.test(value, summer_surf_mean*10, method = "spearman")$p.value,
         p_lab = ifelse(p > 0.001, round(p, 2), p),
         p_lab = paste0("= ", round(p_lab, 3)),
         p_lab = ifelse(p_lab == "= 0",
                        "< 0.001",
                        p_lab)) %>%
  ggplot(aes(x = value*10, y = summer_surf_mean*10))+
  geom_vline(aes(xintercept = 0), color = "grey70")+
  geom_point(shape = 21, stroke = 0.3)+
  #geom_abline()+
  #geom_smooth(method = "lm")+
  geom_label(aes(label = paste0("Spearman's \u03C1 = ", round(rho, 2),
                                "\np ", p_lab), x = -3, y = -2.8, hjust= 0), 
             data = . %>% select(name, rho, p_lab) %>% distinct(), alpha = 0.5)+
  facet_grid(~name)+
  theme_bw()+
  xlab("Air temperature trend (ºC/decade)")+
  ylab("Surface-water temperature\ntrend (ºC/decade)")

air_trends_wide_data <- mem_rf_df_surf_temp %>%
  pivot_longer(cols = all_of(potential_drivers),
               names_to = "var", values_to = "x") %>%
  filter(!is.na(x)) %>%
  mutate(var = factor(var, levels = c("Air_temp_C_spring",
...
jpeg("../Figures/simple_Fig5_surf.jpg", res = 300, units = "in", width = 5, height = 3)
ggpubr::ggarrange(air_trends_wide_blank + theme(plot.margin = unit(c(.2,.2,0,1), 'lines')), 
                    cor_plot + theme(plot.margin = unit(c(0,.2,.2,1), 'lines')), 
                    heights = c(0.2, 1), nrow = 2)
dev.off()
...
responses <- c("summer_vhod")
potential_drivers <- c("Air_temp_C_spring", "Air_temp_C_summer")
mem_rf_df_vhod <- df_for_rf %>%
  select(all_of(c(responses, potential_drivers))) %>%
  na.omit() %>%
  mutate(target = "Bottom-water\noxygen demand")

cor_plot <- mem_rf_df_vhod %>%
  pivot_longer(c(Air_temp_C_spring, Air_temp_C_summer)) %>%
  mutate(name = case_match(name,
                          "Air_temp_C_spring"~"Spring",
                          "Air_temp_C_summer"~"Summer")) %>%
  group_by(name) %>%
  mutate(rho = cor.test(value, summer_vhod*10, method = "spearman")$estimate,
         p = cor.test(value, summer_vhod*10, method = "spearman")$p.value,
         p_lab = ifelse(p > 0.001, round(p, 2), p),
         p_lab = paste0("= ", round(p_lab, 3)),
         p_lab = ifelse(p_lab == "= 0",
                        "< 0.001",
                        p_lab)) %>%
  ggplot(aes(x = value*10, y = summer_vhod*10))+
  geom_vline(aes(xintercept = 0), color = "grey70")+
  geom_point(shape = 21, stroke = 0.3)+
  #geom_abline()+
  #geom_smooth(method = "lm")+
  geom_label(aes(label = paste0("Spearman's \u03C1 = ", round(rho, 2),
                                "\np ", p_lab), x = -3, y = 0.15, hjust= 0), 
             data = . %>% select(name, rho, p_lab) %>% distinct(), alpha = 0.5)+
  facet_grid(~name)+
  theme_bw()+
  xlab("Air temperature trend (ºC/decade)")+
  ylab("Bottom-water oxygen\ndemand trend (ºC/decade)")

air_trends_wide_data <- mem_rf_df_vhod %>%
  pivot_longer(cols = all_of(potential_drivers),
               names_to = "var", values_to = "x") %>%
  filter(!is.na(x)) %>%
  mutate(var = factor(var, levels = c("Air_temp_C_spring",
...
jpeg("../Figures/simple_Fig5_vhod.jpg", res = 300, units = "in", width = 5, height = 3)
ggpubr::ggarrange(air_trends_wide_blank + theme(plot.margin = unit(c(.2,.2,0,1), 'lines')), 
                    cor_plot + theme(plot.margin = unit(c(0,.2,.2,1), 'lines')), 
                    heights = c(0.2, 1), nrow = 2)
dev.off()
