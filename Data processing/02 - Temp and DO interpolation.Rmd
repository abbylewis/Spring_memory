...
Step 4: Interpolate DO concentration, following Jane et al. (2021) - note that this will take up to 30 min to run all lakes
Step 5: Interpolate DO saturation
Step 6: Interpolate temperature - note that this will take up to 30 min to run all lakes
Step 7: Combine DO and temp
...
```{r}
# Import data
#options(timeout=300) may be needed depending on internet speed
...
# Some still have fairly large ranges even after outlier removal. 
# HOWEVER these are mostly very deep lakes
...
to_profiles_depth_checked_ele <- test_depth_removal%>%
  dplyr::select(LakeID, Date, Depth_m, Temp_C, 
                DO_sat, DO_mgL, Flag_Temp_C, Flag_DO_mgL, Elevation_m)

# Fill in missing oxygen saturation and concentration data
to_profiles_oxy_units <- to_profiles_depth_checked_ele %>%
  mutate(Elevation_m = ifelse(is.na(Elevation_m) | Elevation_m < 0, 0, Elevation_m),
         DO_sat = ifelse(is.na(DO_sat),
                         DO.unit.convert(DO_mgL, 
                                         DO.units.in = "mg/L", 
                                         DO.units.out = "pct", 
                                         bar.units.in = "kpa",
                                         bar.press = airpressure_from_elev(Elevation_m), 
                                         temp.C = Temp_C),
                         DO_sat),
         DO_mgL = ifelse(is.na(DO_mgL),
                         DO.unit.convert(DO_sat, 
                                         DO.units.in = "pct", 
                                         DO.units.out = "mg/L", 
                                         bar.units.in = "kpa",
                                         bar.press = airpressure_from_elev(Elevation_m), 
...
to_profiles_depth_checked <- to_profiles_depth_checked_ele %>%
  select(-Elevation_m)
...
# NOTE I looked into this and it is almost entirely from removing lakes without 
# surface measurements

sum(!is.na(to_profiles_depth_checked$DO_mgL))
sum(!is.na(to_profiles_oxy_units$DO_mgL)) #Added a few DO concs

sum(!is.na(to_profiles_depth_checked$DO_sat))
sum(!is.na(to_profiles_oxy_units$DO_sat)) #Added a lot of DO pct saturation
```
...
to_profiles_cleaning1 <- to_profiles_oxy_units %>%
  filter(!is.na(DO_mgL)) %>%
  group_by(LakeID,Date) %>% 
  filter(length(unique(Depth_m)) > 2, #at least three measurement depths
         ) %>%
...
#add additional 0 m measurement for depths between 0.5 and 3
...
# Create functions to map across all lakes
...
#Functions to map across all lakes
...
Step 6: Interpolate temperature
...
#oxygen
to_profiles %>% filter(!is.na(DO_mgL)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() #99882
database.interp %>% filter(!is.na(DO_mgL)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() #83122
#temp
to_profiles %>% filter(!is.na(Temp_C)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() #105804
database.interp %>% filter(!is.na(Temp_C)) %>% group_by(LakeID) %>% dplyr::select(Date) %>% unique() #83786
```
