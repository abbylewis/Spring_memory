...
memory_plot <- memory %>%
  ...
  geom_boxplot()+
  xlab("Relative correlation with\nspring vs. summer air temp")+
  theme_bw() +
...
mem_plot <- memory %>%
  filter(var %in% c("Summer hypo. temperature",
                    "Summer hypo. DO")) %>%
...
mem_lm <- memory %>%
  ...
  rename(epi_temp = `Summer epi. temperature`,
         hypo_temp = `Summer hypo. temperature`,
         hypo_do = `Summer hypo. DO`,
         vhod = `Summer VHOD`,
...
do <- plot_effects_lm(mod_lm,"Relative correlation\nsummer hypo. DO") 
...
comb <- effects2%>%
  ...
  mutate(name = factor(name, 
                       levels = c("hypo_temp",
                                  "hypo_do"),
                       labels = c("Summer hypo. temperature",
                                  "Summer hypo. DO"))) %>%
...
jpeg("../Figures/Memory drivers.jpg", res = 300, width = 5, height = 5, units = "in")
ggarrange(comb,
          mem_plot + theme(axis.title.y = element_text(vjust = -18)),
          nrow = 2, common.legend = T, legend = "bottom", 
          align = "v", heights = c(1,1.3), labels = "auto", label.x = .1)+
