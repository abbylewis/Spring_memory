...
do_temp_roll %>%
  ...
  geom_hline(yintercept=0)+
  geom_line(aes(x = as.Date("2022-01-01") + days(doy), y = monthly_correlation, 
                group = LakeID, color = p < alpha),
            alpha = 0.2, lwd = 0.1)+
  geom_smooth(aes(x = as.Date("2022-01-01") + days(doy), y = monthly_correlation),
              color = "pink")+
  xlab("Air temperature month")+
...
colored_sd <- all_plot1 %>%
  full_join(all_plot2) %>%
  ggplot()+
  #geom_rect(aes(xmin = as_datetime("2022-02-01"), xmax = as_datetime("2022-04-30"), 
  #              ymin = -Inf, ymax = Inf), 
  #              fill = "grey80", alpha = 0.3, 
  #          data = empty_df) +
  #geom_rect(aes(xmin = as_datetime("2022-02-01") + days(30), 
  #              xmax = as_datetime("2022-04-30") + days(30), 
  #              ymin = -Inf, ymax = Inf), 
  #              fill = "grey80", alpha = 0.3, 
  #          data = empty_df) +
