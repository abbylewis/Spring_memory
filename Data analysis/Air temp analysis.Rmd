...
ggarrange(temp_temp_epi_agu + 
            ...
            ggtitle("Summer epilimnetic temperature") +
            ylab("Correlation"),
          temp_temp_agu +
            ggtitle("Summer hypolimnetic temperature") +
            ylab("Correlation"),
...
do_temp_agu <- many_lake_temp_sum %>%
  mutate(hypoxic = ifelse(max(DO_mgL_HYPO, na.rm = T)<1,"yes","no"))%>%
  filter(hypoxic=="no") %>%
  monthly_correlations_talk("DO_mgL_HYPO", "value", "bottom-water DO", 
...
temp_temp_data <- many_lake_temp_sum%>%
  ...
  mutate(strat_Temp_C_HYPO = mean(strat_Temp_C_HYPO, na.rm = T),
         value = mean(value, na.rm = T)) %>%
  filter(!is.na(strat_Temp_C_HYPO),!is.na(value))%>%
...
jpeg("../Figures/temp_temp_sp_su.jpeg", width = 5, height = 5, units = "in", res = 300)
temp_temp_sp_su %>%
  ggplot(aes(y = Spring - Summer, x = buoyancy_freq)) +
  geom_hline(yintercept = 0) +
  geom_point() +
...
jpeg("../Figures/temp_temp_sp_su_regional.jpeg", width = 5, height = 5, units = "in", res = 300)
temp_temp_sp_su %>%
  left_join(lat_long) %>%
  mutate(Region = ifelse(StateOrProvince %in% c("Wisconsin", "Minnesota", "Michigan"),
                         "Midwest",
