...
This file loads temp/DO data from the whole year (not just summer), calculates metrics of stratification, exports stratification onset dates, and exports a compiled file with data during the entire stratified period (used to calculate oxygen demand). All code chunks must be run in order.
...
full %>%
  filter(is.na(as.Date(Date_unif)))
#Remove these NAs
full <- full %>%
  filter(!is.na(Date_unif),
         !is.na(Latitude_DD))
