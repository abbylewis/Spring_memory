...
air_trends_wide_blank <- mem_rf_df_bot_temp %>%
  pivot_longer(cols = all_of(potential_drivers),
               names_to = "var", values_to = "x") %>%
  filter(!is.na(x)) %>%
  mutate(var = factor(var, levels = c("Air_temp_C_spring",
...
p %>%
  ...
  ggplot(aes(x = Temp_C_BOT, y = summer_dif*10))+
  geom_point()+
  geom_smooth()+
