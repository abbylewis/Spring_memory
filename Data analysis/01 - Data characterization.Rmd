...
us_map <- ggplot(data = states) +
  ...
  geom_point(data = for_states, aes(Longitude_DD, Latitude_DD), fill = "#3087D9",shape = 21, color = "white", size = 2.5, alpha  =.2, stroke = .4)+
  theme_bw()+
...
euro_map <- ggplot(data = world) +
  ...
  geom_point(data = for_map, aes(Longitude_DD, Latitude_DD), fill = "#3087D9",shape = 21, color = "white", size = 2.8, alpha  =.5, stroke = .4)+
  theme_bw()+
  annotation_north_arrow(location = "bl", which_north = "true", 
      pad_x = unit(0.2, "in"), pad_y = unit(0.2, "in"),
      style = north_arrow_fancy_orienteering) +
...
wi_map <- ggplot(data = states) +
  ...
  geom_point(data = for_wi, aes(Longitude_DD, Latitude_DD), 
             fill = "#3087D9",shape = 21, color = "white", size = 2, alpha  = .5, stroke = .4)+
  theme_bw()+
  #annotation_north_arrow(location = "bl", which_north = "true", 
  #    pad_x = unit(0.2, "in"), pad_y = unit(0.2, "in"),
  #    style = north_arrow_fancy_orienteering) +
...
jpeg("../Figures/NE Map - just points.jpg", res = 300, width = 3, height = 3, units = "in")
for_states <- for_map#%>%
  #filter(Country == "USA")
states <- ne_states(returnclass = "sf",country = "United States of America")
...
library(ggpubr)
library(patchwork)
jpeg("../Figures/World map with insets.jpg", res = 300, width = 6, height = 5, units = "in")
ggarrange(world_map, 
          ggarrange(wi_map, ne_map, plot_spacer()+theme_void(), 
                    labels = c("b", "c", NA), widths = c(4,5,2), nrow = 1), 
