...
library(ncdf4)
isimip <- nc_open('hydrolakes_id.nc') 
...
write.csv(all_export2, "lakes_for_isimip.csv", row.names = F)
...
all_plot <- all %>%
  group_by(var, doy, sig, Region) %>%
  summarise(mean = mean(monthly_correlation, na.rm = T),
            sd = sd(monthly_correlation, na.rm = T))%>%
  mutate(var2 = paste0(var, Region))

all_plot$group <- 1
for(i in 1:length(unique(all_plot$var2))){
  all_plot$group[all_plot$var2 == unique(all_plot$var2)[i]] <- group_fun(all_plot$sig[all_plot$var2 == unique(all_plot$var2)[i]])
}
...
colored_sd <- all_plot1 %>%
  ...
  scale_fill_manual(values = c("grey40","#0FA9E6"))+
  labs(fill = paste0("p < ",alpha)) +
  facet_grid(cols = vars(var), rows = vars(Region)) +
  theme(legend.position = "bottom",
        strip.background = element_blank())+
  xlim(as_datetime(c("2022-01-31","2022-08-31")))

jpeg("../Figures/Monthly correlations - regional.jpg", width = 8, height = 5, res = 300, units = "in")
colored_sd
dev.off()
```
...
hypo_vars <- all_plot1 %>%
  full_join(all_plot2) %>%
  filter(doy>=30) %>% #remove first 29 days (include previous year)
  filter(var %in% c("Summer hypo. temperature (n = 368)",
                    "Summer VHOD (n = 194)",
                    "Summer hypo. DO (n = 286)")) %>%
  mutate(var = factor(var, 
                      levels = c("Summer hypo. temperature (n = 368)",
                                 "Summer VHOD (n = 194)",
                                 "Summer hypo. DO (n = 286)"),
                      labels = c("Summer bottom-water\ntemperature (n = 368)",
                                 "Summer bottom-water\noxygen demand (n = 194)",
                                 "Summer bottom-water\ndissolved oxygen (n = 286)")),
         layer = ifelse(grepl("epi", var), "epi", "hypo")) %>%
  ggplot() + 
  geom_hline(yintercept=0)+
  geom_vline(xintercept = as_datetime("2022-04-21"), color = "grey50", lty = "dashed")+
...
jpeg("../Figures/Monthly correlations - hypo daily roll mean sd.jpg", width = 6, height = 2.5, res = 300, units = "in")
hypo_vars
dev.off()
```
...
What dates are most correlated
...
library(ggridges)
dates_to_plot %>%
  filter(var %in% c("Summer hypo. DO", "Summer VHOD",
                    "Summer epi. temperature", "Summer hypo. temperature")) %>%
  left_join(lat_long) %>%
  filter(abs(Latitude_DD) > 30) %>%
  mutate(max_doy = ifelse(var == "Summer hypo. DO", min_doy, max_doy)) %>%
  ggplot(aes(x = as.Date("2022-01-01") + max_doy, 
             y = var))+
  geom_rect(xmin = as.Date("2022-01-01") + sp[1], 
            xmax = as.Date("2022-01-01") + sp[2], 
            ymin = -Inf, ymax = Inf, fill = "grey90", alpha = 0.1)+
  geom_rect(xmin = as.Date("2022-01-01") + su[1], 
            xmax = as.Date("2022-01-01") + su[2], 
            ymin = -Inf, ymax = Inf, fill = "grey90", alpha = 0.1)+
  geom_density_ridges(
      quantile_lines = TRUE,
...
write.csv(dates_export, "../Compiled data/strongest_correlation_dates.csv", row.names = F)

dates_export_both <- dates_to_plot %>%
  ungroup() %>%
  filter(grepl("temperature", var))

write.csv(dates_export_both, "../Compiled data/strongest_correlation_dates_both.csv", row.names = F)

dates_export_hypo <- dates_to_plot %>%
  ungroup() %>%
  filter(grepl("hypo", var)) %>%
  filter((grepl("temperature", var)) | #epi
           (grepl("DO", var))) 

write.csv(dates_export_hypo, "../Compiled data/strongest_correlation_dates_hypo.csv", row.names = F)
...
all %>%
  ...
  geom_line()+
  xlab("Air temperature (30-day rolling mean)")+
  ggtitle("Taylor Pond")+
  ylab("Correlation with summer\nepilimnetic temperature")+
  theme_bw()
dev.off()
...
all %>%
  ...
  geom_rect(xmin = su[1], xmax = su[2], ymin = -Inf, ymax = Inf, fill = "grey90", alpha = 0.1)+
  geom_line()+
  geom_point(aes(color=is.max, alpha = is.max), size = 2)+
...
jpeg("../Figures/Monthly correlations - sum long concept v2 many windows.jpg", width = 7, height = 3, res = 600, units = "in")
memory %>%
  filter(!var == "Summer\nepi. DO sat") %>%
  group_by(var, window) %>%
  mutate(n = length(unique(LakeID)),
