...
#filter to rows with 15 years of data
not_na <- full %>%
  filter(if_any(.cols = c(Temp_C, DO_mgL),
                .fns = ~ !is.na(.x))) %>%
  filter(length(unique(year(Date_unif[!is.na(Temp_C)]))) >= 15 |
           length(unique(year(Date_unif[!is.na(DO_mgL)]))) >= 15)
...
not_na%>%
  group_by(LakeID) %>%
  dplyr::summarize(duration = as.numeric(max(Date_unif)-min(Date_unif))/365+1) %>%
  ungroup() %>%
...
min(not_na$Date_unif)
max(not_na$Date_unif)

# Total number of profiles
not_na %>%
  select(LakeID, Date_unif) %>%
  unique()
...
unique(lat_long$Country[lat_long$LakeID %in% not_na$LakeID])
```
