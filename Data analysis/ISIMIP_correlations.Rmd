...
climate <- read.csv("../Compiled data/ISIMIP_GFDL_temp_1981_2005.csv")
summer1 <- read.csv("../Compiled data/ISIMIP_summer_avgs.csv")
summer2 <- read.csv("../Compiled data/ISIMIP_summer_avgs2.csv")
...
all %>%
  ...
  ggplot(aes(x = doy, y = monthly_correlation, color = as.factor(LakeID))) +
  geom_point()
```
...
write.csv(all, "../Compiled data/ISIMIP correlations - 30 day rolling mean.csv", row.names = F)
```

```{r}
alpha <- 0.05
all <- read.csv("../Compiled data/ISIMIP correlations - 30 day rolling mean.csv") %>%
  mutate(sig = ifelse(p < alpha, T, F))

sp <- yday(c("2022-03-01", "2022-06-30"))
...
jpeg("../Figures/ISIMIP monthly correlations - sum long concept v2.jpg", width = 3.5, height = 3, res = 600, units = "in")
memory %>%
  group_by(var) %>%
...
jpeg("../Figures/Monthly correlations - all daily roll mean sd ISIMIP.jpg", width = 8, height = 5, res = 300, units = "in")
colored_sd
dev.off()
...
strat_onset <- read.csv("../Compiled data/ISIMIP_strat_onset2.csv")
...
dates_cor %>%
  ...
  geom_abline(slope = 1) +
  scale_color_gradientn(colors = c("#25469a","white","#c16586"))+
  ylab("Spring date that is most correlated with hypo. temp") +
  xlab("Median stratification onset date")+
