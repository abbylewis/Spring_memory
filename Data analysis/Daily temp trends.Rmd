...
jpeg("../Figures/Temp_trends_by_season.jpeg", width = 3.5, height = 3, units = "in", res = 300)
air_trends+
...
water_trends_matched %>%
  group_by(LakeID) %>%
  filter(sum(!is.na(trend)) == 2) %>%
  mutate(season = ifelse(layer == "summer_hypo_mean",
                         "Summer bottom water",
  ...
  stat_compare_means(label.y = 0.4, label.x = 1.25)+
  theme_bw()+
  theme(axis.title.x = element_blank(),
        axis.text.x = element_text(color = c("#25469a", "#c16586")))

ggarrange(air_trends, water_trends, ncol = 2, nrow = 1, align = "hv")

trends_matched %>%
  dplyr::select(LakeID, season, trend) %>%
  mutate(layer = ifelse(season == "spring",
                        "summer_hypo_mean",
                        "summer_epi_mean")) %>%
  filter(!is.na(trend)) %>%
