...
interp_t <- function(lake, to_profiles_cleaning){
  alldates<-to_profiles_cleaning %>% 
  ...
  lake_data <- to_profiles_cleaning %>%
    filter(hylak_id == lake)
  
  interpolatedvalues.t <- furrr::future_map(alldates, run_all_dates_t, lake_data,lake) |> list_rbind()
...
laken <- to_profiles_cleaning$hylak_id %>% 
  unique()
interpolatedvalues.t <- furrr::future_map(laken,
                                          interp_t,
                                          to_profiles_cleaning) |> 
  list_rbind()
```
