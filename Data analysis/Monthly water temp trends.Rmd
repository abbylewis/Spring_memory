...
jra55_seas <- jra55 %>%
  group_by(LakeID) %>%
  summarize(Spring = mean(trend[Month %in% c(3, 4)]),
            Summer = mean(trend[Month %in% c(8)])) %>%
  mutate(source = "jra55")
era5_seas <- era5 %>%
  group_by(LakeID) %>%
...
density_air <- ALL %>%
  filter(source == "era5",
         !is.na(air_temp) & (!is.na(water_temp)&layer %in% c("summer_epi_mean", "summer_hypo_mean"))) %>%
  ggplot(aes(x = air_temp)) +
  geom_vline(xintercept = 0, color = "grey75") +
  geom_density() +
  facet_grid(cols = vars(season)) +
...
era5_seas %>%
  ...
  pivot_longer(cols = c("summer_epi_mean", "summer_hypo_mean", "Spring", "Summer"),
               names_to = "var", values_to = "temp") %>%
  ggplot(aes(x = var, y = temp)) +
  geom_boxplot() 
```
